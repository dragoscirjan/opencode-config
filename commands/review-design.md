...
2. **Dispatch reviewers** — based on the document type:
   - **Design overview** → @architect + @tech-lead review
   - **HLD** → @tech-lead + relevant dev(s) review
   - **LLD** → relevant dev(s) review (not @tech-lead if they authored it)
...
Follow standard wire-protocol review conventions. Use `spec-naming` for all paths.
