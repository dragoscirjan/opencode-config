...
2. **Dispatch reviewers** — based on the document type:
   - **Design overview** → @worker-sys-architect + @worker-tech-lead review
   - **HLD** → @worker-tech-lead + relevant dev(s) review
   - **LLD** → relevant dev(s) review (not @worker-tech-lead if they authored it)
