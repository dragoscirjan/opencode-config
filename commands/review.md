...
Dispatch @reviewer to perform a thorough code review.
...
2. **Review** — @reviewer checks: correctness, security (input validation, no hardcoded secrets, auth), performance (no N+1, unnecessary allocations), test coverage, code quality (readability, codebase patterns).
