...
async function generateGemini(
  ...
  try {
    ...
    for (const part of candidates[0].content?.parts ?? []) {
      const inlineData = (part as Record<string, unknown>).inlineData as { data: string; mimeType: string } | undefined;
      if (inlineData?.data) {
...
export default tool({
  ...
  args: {
    command: tool.schema.string().describe('Command: "image", "video", "set_budget", "get_budget"'),
    prompt: tool.schema.string().describe('Generation prompt (required for image/video)').optional(),
    model: tool.schema
      .string()
      .describe('Image backend: "gemini" (5-15c, precise) or "grok" (2c, fast). Default: "grok"')
      .optional(),
    size: tool.schema.string().describe('Resolution. Gemini: 512, 1K, 2K, 4K. Grok: 1K, 2K. Default: "1K"').optional(),
    aspectRatio: tool.schema.string().describe('Aspect ratio, e.g. "1:1", "16:9". Default: "1:1"').optional(),
    image: tool.schema.string().describe('Reference image path for image-to-image or video starting frame').optional(),
    output: tool.schema.string().describe('Output file path (required for image/video)').optional(),
    duration: tool.schema.string().describe('Video duration in seconds, 1-15. Required for video command.').optional(),
    resolution: tool.schema.string().describe('Video resolution: "480p" or "720p". Default: "720p"').optional(),
    cents: tool.schema.string().describe('Budget amount in cents (for set_budget command)').optional(),
  },
    ...
    if (command === 'image') {
      ...
      if (backend === 'gemini') {
        if (!GEMINI_SIZES.includes(size)) {
          return JSON.stringify({
            ok: false,
            error: `Gemini does not support size ${size}. Use: ${GEMINI_SIZES.join(', ')}`,
          });
        }
        ...
        if (!GROK_SIZES.includes(size)) {
          return JSON.stringify({
            ok: false,
            error: `Grok does not support size ${size}. Use: ${GROK_SIZES.join(', ')}`,
          });
        }
      } else {
