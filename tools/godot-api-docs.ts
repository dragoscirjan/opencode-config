...
function parseClass(xml: XmlClass, methodDesc: DescMode): string | null {
  ...
  if (signals.length > 0) {
    ...
    for (const s of signals) {
      ...
      const paramStr = params.length > 0 ? `(${params.map((p) => `${p['@_name']}: ${p['@_type']}`).join(', ')})` : '';
...
export default tool({
  ...
  args: {
    ...
    className: tool.schema.string().describe('Class name for lookup command, e.g. "CharacterBody2D"').optional(),
    filter: tool.schema
      .string()
  ...
  async execute(args, context) {
    ...
    async function ensureDocs(): Promise<{ ok: boolean; path: string; error?: string }> {
      ...
      try {
        ...
        if (!existsSync(join(repoDir, '.git'))) {
          ...
          await git.clone('https://github.com/godotengine/godot.git', repoDir, [
            '--depth',
            '1',
            '--filter=blob:none',
            '--sparse',
    ...
    if (command === 'build') {
      ...
      let xmlFiles = readdirSync(xmlDir)
        .filter((f) => f.endsWith('.xml'))
        .sort();
      if (classFilter) {
      ...
      for (const file of xmlFiles) {
        ...
        if (unifiedSet.has(name)) {
          const brief = xmlClass.brief_description?.['#text'] ? firstSentence(xmlClass.brief_description['#text']) : '';
