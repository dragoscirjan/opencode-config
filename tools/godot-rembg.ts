...
function computeAlphaColor(rgb: Buffer, width: number, height: number, bg: [number, number, number]): Float32Array {
...
export default tool({
  ...
  args: {
    input: tool.schema.string().describe('Input image path (single mode) or directory path (batch mode with --batch)'),
    output: tool.schema
      .string()
    ...
    batch: tool.schema
      ...
      .optional(),
    mode: tool.schema.string().describe('Regime: "auto", "trust", "adapt", "color". Default: "auto"').optional(),
    bgThresh: tool.schema.string().describe('Background threshold override (0-1). Default: per-regime').optional(),
    fgThresh: tool.schema.string().describe('Foreground threshold override (0-1). Default: per-regime').optional(),
  },
  async execute(args, context) {
    ...
    async function processImage(
      ...
      // 7. Save
      await sharp
        .default(rgba, { raw: { width, height, channels: 4 } })
        .png()
        .toFile(outPath);
