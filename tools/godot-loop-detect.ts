...
async function embed(sharp: typeof import('sharp'), filePath: string): Promise<Float32Array> {
...
function dedupe(candidates: Array<[number, number]>, minGap: number): Array<[number, number]> {
...
export default tool({
  ...
  args: {
    framesDir: tool.schema.string().describe('Directory containing numbered frame PNGs (sorted alphabetically)'),
    skip: tool.schema.string().describe('Skip first N frames to avoid intro artifacts. Default: 10').optional(),
    minGap: tool.schema.string().describe('Minimum frames between candidate loop points. Default: 5').optional(),
  },
