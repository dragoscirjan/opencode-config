...
5. Write compressed design overview to **provided path**
6. Self-check: lines ≤150? No code? → `SIG:DONE|PATH:<path>`
7. Revision: read one review → targeted edits → signal done
8. Finalization (when told): follow Finalization section in `wire-design` → translate → write to **provided path**
