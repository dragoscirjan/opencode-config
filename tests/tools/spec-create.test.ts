...
describe('spec-create tool', () => {
  ...
  const runTool = (args: Parameters<typeof specCreateTool.execute>[0], directory = '/mock/dir') =>
    specCreateTool.execute(args, { directory } as Parameters<typeof specCreateTool.execute>[1]);
  ...
  it('should create the first spec as 00001 v1 if .specs does not exist', async () => {
    ...
    expect(writeFileSync).toHaveBeenCalledWith(expectedFilepath, expect.stringContaining('id: "00001"'), 'utf-8');
  ...
  it('should bump version if an ID is provided', async () => {
    ...
    expect(writeFileSync).toHaveBeenCalledWith(expectedFilepath, expect.stringContaining('version: 3'), 'utf-8');
