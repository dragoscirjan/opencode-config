...
describe('godot-visual-qa tool', () => {
  ...
  const runTool = (args: Parameters<typeof godotVisualQaTool.execute>[0], directory = '/mock/dir') =>
    godotVisualQaTool.execute(args, { directory } as Parameters<typeof godotVisualQaTool.execute>[1]);
  ...
  it('should auto-detect static mode and call Gemini API', async () => {
    ...
    expect(callArgs.contents[0].parts).toContainEqual(
      expect.objectContaining({ text: expect.stringContaining('visual target') }),
    );
    expect(callArgs.contents[0].parts).toContainEqual(
      expect.objectContaining({ inlineData: { mimeType: 'image/png', data: 'ZmFrZS1pbWFnZS1kYXRh' } }),
  ...
  it('should auto-detect dynamic mode for multiple images', async () => {
    ...
    const callArgs = mockGenerateContent.mock.calls[0][0];
    expect(callArgs.contents[0].parts).toContainEqual(
      expect.objectContaining({ text: expect.stringContaining('frame sequence') }),
    );
  });
